...
export class AdoptedanimalComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    public servicePet: AdoptedAnimalService,
    fb: FormBuilder,
    public serviceGuardian: GuardianService,
    fb2: FormBuilder
  ) {
  ...
  ngOnInit(): void {
    // this.initData();
    this.loadAll();
    const animalListDiv = <HTMLElement>(
      document.querySelector("#animal-list-table")
    );
  ...
  loadAll(): Promise<any> {
    const tempList = [];
    return this.servicePet
      .getAllByStatusUnProtected("AAA") //getAll() //
      .toPromise()
      .then((result) => {
  ...
  updateAsset(form: any): Promise<any> {
    ...
    return this.servicePet
      .updateAsset(form.get("petSerial").value, this.asset)
      .toPromise()
      .then(() => {
        this.errorMessage = null;
        this.loadAll();
  ...
  updateAnimalDetails(form: any): Promise<any> {
    ...
    return this.servicePet
      .updateAsset(form.get("petSerial2").value, this.asset)
      .toPromise()
      .then(() => {
        this.errorMessage = null;
        this.loadAll();
  ...
  updateAnimalGuardian(form: any): Promise<any> {
    ...
    return this.serviceGuardian
      .updateAsset(form.get("guardianCode").value, this.guardianAsset)
      .toPromise()
      .then(() => {
        this.guardianErrorMessage = null;
        this.loadAll();
